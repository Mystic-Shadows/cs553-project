...
export async function patchTasks(_req: any, _res: any) {
	...
	try {
		const response = await updateTask(id, title, description, status);
		if (response === null) {
...
async function updateTask(id: any, title: any, description: any, status: any) {
	...
	query += `, updated_at=NOW() WHERE id=$${nextParamCount} RETURNING id::int, title, description, status, created_at AS "createdAt", updated_at AS "updatedAt"`
	queryValues.push(id);
